...
class EntryProtocol(discord.ui.View):
    ...
    async def grant(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if role:
            ...
            embed = discord.Embed(title="✅ ACCESS GRANTED", description=f"**{self.member.name}** verified.", color=0x00ff00)
...
async def cc(ctx, name: str, *, info: str):
    ...
    if bot.get_command(name):
        return await ctx.send(f"⚠️ Protocol Error: `{name}` is a core command.")
    cursor.execute("INSERT OR REPLACE INTO custom_cmds VALUES (?, ?)", (name, info))
    db.commit()
    await ctx.send(f"🧬 **Command Synthesized:** `!{name}`")

@bot.command()
async def ccl(ctx):
    cursor.execute("SELECT name FROM custom_cmds")
    rows = cursor.fetchall()
    cmds_list = "\n".join([f"• !{r[0]}" for r in rows]) if rows else "No custom commands."
...
@bot.command()
async def ping(ctx):
    await ctx.send(f"🛰️ **Signal:** {round(bot.latency * 1000)}ms | **Port 8080:** Active")
...
async def on_message_delete(msg):
    ...
    if logs and not msg.author.bot:
        await logs.send(f"🗑️ **Log:** Message by {msg.author.name} deleted: `{msg.content}`")
